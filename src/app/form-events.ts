...
function valueEvents$<T>(form: AbstractControl<T>) {
    return form.events.pipe(
        filter(
            (
                event: ControlEvent,
            ): event is ValueChangeEvent<typeof form.value> =>
    ...
    return defer(() =>
        combineLatest([
            ...
            map(([valueParam, statusParam, touchedParam, pristineParam]) => {
                ...
                const stat: FormControlStatus | StatusChangeEvent =
                    isStatusEvent(statusParam)
                        ? statusParam.status
                        : statusParam;
                const touch: boolean | TouchedChangeEvent = isTouchedEvent(
                    touchedParam,
                )

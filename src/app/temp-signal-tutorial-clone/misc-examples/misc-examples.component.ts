...
@Component({
    ...
    imports: [
        ...
        ComparatorTypesComponent,
        AsyncPipe,
        HttpAndSignalsComponent,
    ],
    template: `
        <div id="double-example">
            <h2>Setting Signal Values</h2>
            <div class="text">
                Updating a signal with
            ...
            <div class="text">
                ...
                <pre highlight-js lang="typescript">computed()</pre>
                any time
                <pre highlight-js lang="typescript">$value</pre>
                changes.
            </div>
            <button
                mat-flat-button
            ...
            <button mat-flat-button color="primary" (click)="updateValue()">
                $value.update(v => v + 1)
            </button>
            <div class="code-bg">
                <pre highlight-js lang="typescript">
$value = signal(0); // {{ value() }}</pre
                >
                <pre highlight-js lang="typescript">
$doubleValue = computed(() => this.$value() * 2); // {{ $doubleValue() }}</pre
                >
            </div>
        </div>
        <app-signal-input />
